...
Iridium_Tracker_Message_Fields::Iridium_Tracker_Message_Fields(void)
{
}

void Iridium_Tracker_Message_Fields::begin()
{
}

// debug functions taken from the SparkFun u-blox library:
// https://github.com/sparkfun/SparkFun_Ublox_Arduino_Library
void Iridium_Tracker_Message_Fields::enableDebugging(Stream &debugPort)
{
...
void Iridium_Tracker_Message_Fields::disableDebugging(void)
{
  _printDebug = false; //Turn off extra print statements
}

void Iridium_Tracker_Message_Fields::debugPrint(char *message)
...
void Iridium_Tracker_Message_Fields::debugPrintln(char *message)
//Safely print messages
{
  if (_printDebug == true)
  {
...
byte Iridium_Tracker_Message_Fields::calculateEEPROMchecksumA() // Calculate the RFC 1145 Checksum A for the EEPROM data
{
  uint32_t csuma = 0;
...
byte Iridium_Tracker_Message_Fields::calculateEEPROMchecksumB() // Calculate the RFC 1145 Checksum B for the EEPROM data
{
  uint32_t csuma = 0;
  uint32_t csumb = 0;
...
bool Iridium_Tracker_Message_Fields::checkEEPROM(trackerSettings *myTrackerSettings)
...
void Iridium_Tracker_Message_Fields::updateEEPROMchecksum() // Update the two EEPROM checksum bytes
{
  byte csuma = calculateEEPROMchecksumA();
  byte csumb = calculateEEPROMchecksumB();
  EEPROM.write(LOC_CSUMA, csuma);
  EEPROM.write(LOC_CSUMB, csumb);
  debugPrint("Iridium_Tracker_Message_Fields::updateEEPROMchecksum: checksums updated: ");
  _debugSerial->print(csuma, HEX);
  debugPrint(" ");
  _debugSerial->println(csumb, HEX);
}

void Iridium_Tracker_Message_Fields::initTrackerSettings(trackerSettings *myTrackerSettings) // Initialises the trackerSettings in RAM with the default values
{
  ...
  debugPrintln("Iridium_Tracker_Message_Fields::initTrackerSettings: RAM tracker settings initialised");
}

void Iridium_Tracker_Message_Fields::putTrackerSettings(trackerSettings *myTrackerSettings) // Write the trackerSettings from RAM into EEPROM
{
  ...
  debugPrintln("Iridium_Tracker_Message_Fields::putTrackerSettings: tracker settings stored in EEPROM");
}

void Iridium_Tracker_Message_Fields::updateTrackerSettings(trackerSettings *myTrackerSettings) // Update any changed trackerSettings in EEPROM
{
  ...
  updateEEPROMchecksum();
  debugPrintln("Iridium_Tracker_Message_Fields::updateTrackerSettings: EEPROM tracker settings updated");
}

void Iridium_Tracker_Message_Fields::getTrackerSettings(trackerSettings *myTrackerSettings) // Read the trackerSettings from EEPROM into RAM
{
  ...
  debugPrintln("Iridium_Tracker_Message_Fields::getTrackerSettings: tracker setings loaded from EEPROM to RAM");
}

void Iridium_Tracker_Message_Fields::displayEEPROMcontents() // Display the EEPROM data nicely
{
...
uint8_t Iridium_Tracker_Message_Fields::ascii_hex_to_bin(uint8_t chr)
...
// RX_IDLE_TIMEOUT in receiving the data
{
  ...
  {
    serial_rx_buffer_size = 0; // Reset the buffer size
  ...
  while (Serial.available()) // Has any serial data been received?
  {
    ...
    serial_rx_buffer[serial_rx_buffer_size++] = Serial.read(); // Read and store one byte from the Serial rx buffer
  }

  if (millis() - rx_start >= 1000UL * CHECK_SERIAL_TIMEOUT) // Check if we have timed out
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_for_serial_data: timed out");
    return (DATA_TIMEOUT);
  }

  if (!data_seen) // If we have not seen any data
  {
  ...
  debugPrintln("Iridium_Tracker_Message_Fields::check_for_serial_data: data received");
  return (DATA_RECEIVED);
}

bool Iridium_Tracker_Message_Fields::is_ID_valid(uint8_t ID, uint16_t &data_width)
...
// Serial data could be binary or ASCII-encoded Hex
{
  ...
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_data: data too short");
  ...
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_data: data is ASCII Hex");
    size_t pointer = 0;
  ...
  if (data_buffer[0] != 2)
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_data: no STX");
    return (NO_STX);
  }
  ...
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_data: no ETX");
    return (NO_ETX);
  }
  ...
  uint32_t csuma = 0;
  uint32_t csumb = 0;
  for (size_t x = 0; x < (data_buffer_size -2); x += 1) // Calculate a sum of bytes and sum of sums for every byte from STX to ETX
  {
  ...
  {
    debugPrint("Iridium_Tracker_Message_Fields::check_data: checksum error ");
    _debugSerial->print(csuma, HEX);
    debugPrint(" ");
  ...
  if (data_buffer_size == 4)
  {
    debugPrintln("Iridium_Tracker_Message_Fields::check_data: message is valid but contains no data");
    return (DATA_VALID);
  }
  ...
  while (x < (data_buffer_size - 2)) // Check each byte except the checksum
  {
    ...
    if (is_ID_valid(data_buffer[x], data_width)) // Check if the field ID is valid
    {
      ...
      {
        debugPrintln("Iridium_Tracker_Message_Fields::check_data: invalid data width");
        return (DATA_WIDTH_INVALID);
      }
    }
    else
    {
      debugPrint("Iridium_Tracker_Message_Fields::check_data: invalid field ID ( ");
...
void Iridium_Tracker_Message_Fields::printTrackerSettings(trackerSettings *myTrackerSettings)
// Print the tracker settings
{
  ...
  _debugSerial->print("HDOP: ");
  _debugSerial->println(((float)myTrackerSettings->HDOP.the_data / 100),2);
...
void Iridium_Tracker_Message_Fields::printBinary(uint8_t the_byte)
